...
$$
\begin{align}
\log p(\tilde{y}_i) &= \mathbb{E} \log \left[p(\tilde{y}_i | \theta)\right] \\
&= \log \int p(\tilde{y_i} | \theta) p(\theta)\,d\theta.
\end{align}
$$
...
scatter!(p1, collect(1:5) .+ 0.2, d_20[:, 1] .+ 2 * (1:5), yerr=d_20[:, 2], lw=2, markersize=5, color=:red, linecolor=:black, label="AIC")

scatter!(p2, collect(1:5) .+ 0.2, d_100[:, 1] .+ 2 * (1:5), yerr=d_20[:, 2], lw=2, markersize=5, color=:red, linecolor=:black, label="AIC")
...
::: {.fragment .fade-in}
BIC vs. AIC is the tradeoff between causal vs. predictive analyses. **Generally not coherent to use both for the same problem**.
:::
