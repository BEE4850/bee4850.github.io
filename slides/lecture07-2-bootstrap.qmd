...
- Monte Carlo is an unbiased estimator, but beware (and always report) the standard error.
...
But is there a more general approach?
...
Assumes the existing data is representative of the "true" population, and can simulate based on properties of the data itself, then re-estimate statistics of interest from re-samples.
...
- **Bias Corrected and Accelerated CIs (BCa)** (use for bias-corrected and skewed distributions): complicated formula, relies on transforms between normal CDF and bootstrapped statistic CDF as well as estimates of "acceleration" parameter.
